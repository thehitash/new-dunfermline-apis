...
export const requestRide = async (req: IRequest, res: Response) => {
  const { pickup, destination, vehicleId, estimatedFare, bookingMethod } = req.body;
...
export const allDriverRides = async (req: IRequest, res: Response) => {
  ...
  if (req.user.userType !== 'driver') {
    return res.status(403).json({ message: 'Drivers only', status: false });
  }
...
export const allRiderRides = async (req: IRequest, res: Response) => {
  ...
  try {
    ...
    if (isDriver && req.user.userType !== 'driver') {
      return res.status(403).json({ message: 'Drivers only view', status: false });
    }

    const userField = isDriver ? 'driver' : 'rider';
    const query: any = {
...
export const pendingRides = async (req: IRequest, res: Response) => {
  try {
    ...
    if (req.user.userType !== 'driver' && !req.user.isAdmin) {
      return res.status(403).json({ message: 'Drivers only', status: false });
    }

    const { lat, lng, radius } = req.query;

    console.log('🔍 Fetching pending rides for drivers', { lat, lng, radius });

    // Base query filters
    const query: any = {
      status: 'pending',
      paymentStatus: 'completed', // Only show paid rides
      bookingMethod: { $ne: 'whatsapp' } // Exclude WhatsApp bookings
    };

    // Add location-based filtering if driver location is provided
    if (lat && lng) {
      const driverLat = parseFloat(lat as string);
      const driverLng = parseFloat(lng as string);
      const searchRadius = radius ? parseFloat(radius as string) : 20; // Default 20km radius

      // Convert km to meters for MongoDB
      const radiusInMeters = searchRadius * 1000;

      // Use $geoWithin to find rides within radius
      query['pickupLocation.coordinates'] = {
        $geoWithin: {
          $centerSphere: [[driverLng, driverLat], radiusInMeters / 6378100] // Earth radius in meters
        }
      };

      console.log(`📍 Filtering rides within ${searchRadius}km of driver location (${driverLat}, ${driverLng})`);
    } else {
      console.log('🌍 No location filter - showing all rides');
    }
    ...
    console.log(`✅ Found ${rides.length} pending rides with completed payment (WhatsApp bookings excluded)`);
...
export const getRideById = async (req: IRequest, res: Response) => {
  ...
  try {
    ...
    const isParticipant =
      requesterId &&
      ((ride.rider as any)._id?.toString() === requesterId ||
        (ride.driver as any)?._id?.toString() === requesterId);

    if (!isParticipant && !req.user?.isAdmin) {
      return res.status(403).json({ message: 'Not authorized to view this ride', status: false });
    }
