...
/**
 * Admin: update any user's profile details
 */
export const updateUserByAdmin = async (req: IRequest, res: Response) => {
  if (!req.user?.isAdmin) {
  ...
  try {
    ...
    if (phoneNumber && phoneNumber !== user.phoneNumber) {
      const phoneExists = await User.findOne({ phoneNumber, _id: { $ne: userId } });
      if (phoneExists) {
        return res.status(400).json({
    ...
    if (user.userType === 'driver' || userType === 'driver') {
      if (!user.carDetails) {
        user.carDetails = {
          carName: '',
          carNumber: '',
